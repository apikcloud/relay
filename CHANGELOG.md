...
- `CartographerConfig.from_env()` raised `TypeError` instead of falling back to
  the default base URL: with `slots=True`, `cls.base_url` returns the slot
  descriptor, not the field's default string. Introduced a `DEFAULT_BASE_URL`
  module constant and referenced that instead.

## [0.9.0] - 2026-09-18

### Added

- `GithubClient.create_tree` — creates a tree object from raw GitHub
  tree-entry dicts, optionally layered on a `base_tree`; the robust way to
  *remove* a root-level entry (e.g. a `160000` submodule gitlink) since
  `sha: null` deletion is only documented for blobs
- `GithubClient.create_commit` — creates a commit object pointing at a
  tree with given parents, without moving any ref
- `relay.cartographer` — new submodule (`CartographerConfig`,
  `CartographerClient`) reading Cartographer's read-only catalog API:
  `custom_repo_detail`, `module_detail`, `live_refs`, `resolve_expand`,
  `base_images`

### Changed

- `GithubClient.list_tree` gains a `recursive: bool = True` keyword-only
  parameter; `recursive=False` returns only the immediate children of the
  given sha instead of the full recursive tree. Existing positional
  two-arg calls are unaffected.
